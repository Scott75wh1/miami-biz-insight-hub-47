...
// Mock data for the Places API
type BusinessTypes = 'restaurant' | 'cafe' | 'coffee_shop' | 'food' | 'bar' | 'tech' | 'business_center' | 'gym' | 'fitness' | 'retail' | 'shopping' | 'clothing' | 'specialty' | 'art' | 'dance' | 'coworking' | 'fine_dining' | 'seafood' | 'cuban' | 'bistro' | 'establishment';
...
interface MockDistrictData {
  [key: string]: MockPlace[];
}

// Mock data for different districts in Miami
export const districtBusinesses: MockDistrictData = {
  'Downtown': [
    { 
...
// Generate mock places data based on query and location
export const generateMockPlacesData = (query: string, location: string = 'Miami, FL') => {
  console.log(`Generating mock data for query: "${query}" in ${location}`);
...
// Generate basic mock data when no specific district is found
export const generateBasicMockData = (query: string, location: string) => {
